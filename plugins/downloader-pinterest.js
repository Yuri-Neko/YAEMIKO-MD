...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	text.match(URL_REGEX) ?
		await conn.sendMessage(m.chat, { [mime.split('/')[0]]: { url: res }, caption: `Sukses Memproses: ${await shortUrl(res)}` }, { quoted: m }) :
	await conn.sendButton(m.chat, `Pencarian Dari: ${text.capitalize()}`, await shortUrl(res), res, [['Makasih', `ok`]], m)
}
