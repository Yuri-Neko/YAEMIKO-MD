...
export async function before(m) {
	...
	if (chat && chat.updateAnimeNews) {
		...
		setInterval(async () => {
			this.logger.info(`Memeriksa Anime Terbaru Untuk "${m.chat}"`)
			let res = await getNews().catch(console.error)
			let { id, sourceUrl, url, caption } = res
			if (latest.includes(id)) return this.logger.info(`ID ${id} Siap Di Kirim Ke "${m.chat}"`)
			let length = latest[latest.length - 1]
			latest.push(id)
			if (latest.indexOf(length) !== -1) latest.splice(latest.indexOf(length), 1)
			this.logger.info(`Mengirim Anime Terbaru Ke "${m.chat}"`)
