...
async function handler(m, { conn, args, usedPrefix, command }) {
    ...
    let lol = `Contoh ${usedPrefix}${command} money 9999 @621927237001
...
`.trim()
    ...
    if (user[type] * 1 < count) return m.reply(`Kamu *${rpg.emoticon(type)}${type}${special(type)}* Kurang *${count - user[type]}*`)
    let confirm = `
*––––––『 TRANSFER 』––––––*
